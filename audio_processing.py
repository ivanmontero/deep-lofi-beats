...
def get_batch(data, max_len, batch_size, device, segment_size=1, full=False):
    ...
    if not full:
        ...
        min_len = ((min([batch[i].shape[0] for i in range(batch_size)]) - max_len*segment_size) // segment_size) - 1
        n_start = max_len*segment_size
        n_end = min_len*segment_size + n_start
        p, n = np.zeros((batch_size, n_start)), np.zeros((batch_size, min_len*segment_size))
        for i in range(batch_size):
            p[i] = batch[i][:n_start]
            n[i] = batch[i][n_start:n_end]
        return torch.tensor(p).to(device).float(), torch.tensor(n).to(device).float()
